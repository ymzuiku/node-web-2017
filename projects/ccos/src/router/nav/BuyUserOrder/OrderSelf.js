...
class OrderGive extends React.Component {
  constructor(props) {
    ...
    inputs[0].tel = sos.user.tel || ''
    inputs[0].name = sos.user.name || ''
    inputs[0].idcard = sos.user.idcard||''
    this.state = {
      loading:true,
  ...
  componentDidMount() {
    ...
    if (sos.user.idcard && window.q.regIdcardInfo(sos.user.idcard).age < 29.5 && !window.agemin) {
      window.agemin = true
      window.msg({ info: '您的年龄小于30岁,不合适购买该服务' })
    }
  ...
  update = (_inputs, event) => {
    ...
    api.save_order(
      ...
      res => {
        // window.msg({ info: JSON.stringify(res) })
        this.setState({
          oid:res.orderid
        }, () => {
          if (event) {
  ...
  send = () => {
    ...
    for (let i = 0, len = inputs.length; i < len; i++) {
      ...
      if (!ele.name || ele.name.length < 2) {
        ...
        window.msg({ info: window.q.regIdcard(ele.idcard) })
        let obj = document.getElementById('idcard' + num)
        if(obj) obj.focus()
        return
      }
      else if (window.q.regIdcardInfo(ele.idcard).age <29.5) {
        window.msg({ info: '您的年龄小于30岁,不合适购买该服务' })
        return
      }
      ...
      else if (ele.pcd === defAddress) {
        window.msg({ info: `请选择地区` })
        return
      } else if (ele.addr === '') {
        let obj = document.getElementById('addr' + num)
        if(obj) obj.focus()
        window.msg({ info: `请输入详细地址` })
        return
      }
      // else if (!this.iagreeChange) {
      //   window.msg({ info: '请阅读并同意知情同意书' })
      //   return
      // }
      else {
        _inputs.push(ele)
    ...
    if (_inputs.length === 0) {
      ...
      if (!sos.user.idcard) {
        api.updateUserInfo({ idcard: _inputs[0].idcard, name: _inputs[0].name }, (res) => {
  ...
  render() {
    let { type, data, pc, num, fixMoney, citys, inputs } = this.state
    let lookproduck = this.props.lock
    let sos = so.getState()
    ...
    return (
      <div style={{  backgroundColor: colors.white3 }}>
          ...
          <div style={{flexDirection:'row', justifyContent:'flex-start', flex:6  }}>
            ...
            {type === 'self' ? null : (
              <div style={{justifyContent:'center', marginRight: 20,}}>
                <NumberEditor disabled={lookproduck} min={1} num={num} onChang={this.numbuers} />
              </div>
            )}
          </div>
        </CpPlan>
        ...
        <Steps current={lookproduck?-1:0} style={{ flexDirection:'row', justifyContent: 'center', width: '90%', }}>
            <Step title="在线下单" description={pc ? "在线购买合适自己的服务":''} />    
