...
if (window.low || !window.requestAnimationFrame) {
...
Comp = class _Comp extends React.PureComponent {
  ...
  renderItem = (ele, i) => {
    ...
    if (full) {
      return <div key={'banner'+i} style={{ width: this.rect.width, height: this.rect.height }}>{ele}</div>
    }
    return ele
  }
  renderChildren = len => {
    ...
    this.arr = []
    if (infinite) {
      this.arr.push(this.renderItem(children[children.length - 1], -2))
    }
    for (let i = 0; i < children.length; i++) {
      let ele = children[i]
      this.arr.push(this.renderItem(ele, i))
    }
    if (infinite) {
      this.arr.push(this.renderItem(children[0], -1))
    }
    return this.arr
  }
  render() {
    ...
    return (
      <div
        ...
        <Motion
          ...
          {mot => <div
            style={{
            flexDirection:scrollXY === 'x'? 'row':'column',
